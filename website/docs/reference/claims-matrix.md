...
| NestJS `^11.0.0 \|\| ^12.0.0` — both ends of the range run the suite and every sample. | `.github/workflows/ci.yml` (default legs on the lockfile's 11.x; the `nestjs-compat` matrix on `11.0.0` exactly and `^12`), `scripts/check-nestjs-resolution.mjs` | Covered by CI on both ends of the peer range; each leg fails on any peer conflict npm merely warned about and proves the exact version every workspace resolves before running. |
